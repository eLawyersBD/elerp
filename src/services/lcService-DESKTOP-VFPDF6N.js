...
export const lcService = {
  getLCs: () => {
    // Background sync
    fetch(`${BACKEND_URL}/erp/lcs`)
      .then(res => {
        if (res.ok) return res.json();
      })
      .then(data => {
        if (data) saveLocal('erp_lc_records', data);
      })
      .catch(err => console.warn('[lcService] MySQL fetch LCs background failed', err.message));
  ...
  saveLC: (lc) => {
    ...
    .catch(err => console.warn('[lcService] MySQL saveLC sync failed', err.message));
  ...
  deleteLC: (id) => {
    ...
    // Background delete
    fetch(`${BACKEND_URL}/erp/lcs/${id}`, {
      method: 'DELETE'
    })
    .catch(err => console.warn('[lcService] MySQL deleteLC sync failed', err.message));

    return true;
  },

  postLCMargin: async (lcId, amount, type, refNo, bankAccountId, currentUser) => {
    const lcs = lcService.getLCs();
  ...
  runCostAllocation: (lcId, allocationBasis) => {
    const lcs = lcService.getLCs();
    const lc = lcs.find(item => item.id === lcId);
    if (!lc) throw new Error('LC record not found');
  ...
  saveCustomsAssessment: (lcId, customsForm) => {
    const lcs = lcService.getLCs();
    const lc = lcs.find(item => item.id === lcId);
    if (!lc) throw new Error('LC record not found');
  ...
  capitalizeLC: async (lcId, warehouseId, currentUser) => {
    ...
    if (!hasAllocated) {
      lcService.runCostAllocation(lcId, 'FOB Value');
      const reloaded = lcService.getLCs();
    ...
    // Background product inventory updating in MySQL
    fetch(`${BACKEND_URL}/erp/products`)
      .then(res => {
        if (res.ok) return res.json();
      })
      .then(products => {
      ...
      .catch(pErr => console.warn('[lcService] Background products capitalization failed', pErr.message));
